...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    if [[ -n "$custom_dest" ]]; then
        ...
        # Correct handling: If the source is under $HOME, remove only '/home/$USER' part
        if [[ "$source" == "/home/$USER"* ]]; then
            rel_path="${source#/home/$USER/}"  # Remove leading '/home/$USER'
            destination="$MACHINE_DIR/.config/$rel_path"
        else
            # Preserve full absolute path for other files (Option 1)
